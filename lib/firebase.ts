...
/**
 * Inserts a partner lead into the `partner_leads` Firestore collection.
 *
 * Unlike waitlist leads, partner submissions are not deduped — the same
 * company may submit multiple briefings over time. We use addDoc to append
 * a new document on every valid submission.
 */
export async function insertPartnerLead(
  lead: PartnerLeadInsert
  ...
  try {
    await addDoc(collection(db, 'partner_leads'), {
      ...
      locale: lead.locale,
      source: lead.source ?? 'website-partner',
      createdAt: serverTimestamp(),
    });
    return { ok: true };
  } catch (e) {
    // eslint-disable-next-line no-console
    console.error('[insertPartnerLead] Firebase write failed:', e);
    const msg = e instanceof Error ? `${e.name}: ${e.message}` : 'unknown';
    return { ok: false, error: msg };
  }
}
